...
//   const mealPlan = useMemo(() => parseMealPlanFromMessage(content), [content]);
...
//       // Add system prompt for meal planning
...
//             {item.content.replace(/```json[\s\S]*?```/g, '').replace(/\{[\s\S]*?"type"\s*:\s*"meal_plan"[\s\S]*?\}/g, '').trim()}

//           </Markdown>
//         </View>
//         <View style={styles.messageActions}>
//           <CopyButton content={item.content.replace(/```json[\s\S]*?```/g, '').replace(/\{[\s\S]*?"type"\s*:\s*"meal_plan"[\s\S]*?\}/g, '').trim()} />

//           <MealPlanButton content={item.content} />
//         </View>
