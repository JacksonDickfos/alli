...
const ANALYSIS_ICON = require('../assets/Chick2.png');
...
export default function NutritionScreen({ navigation, route }: NutritionScreenProps) {
  const { state, getCurrentDayLog, getTodaysTotals, addFoodItem, removeFoodItem, addHydrationEntry, addBowelEntry, removeBowelEntry, addSymptomEntry, removeSymptomEntry, addExerciseEntry, removeExerciseEntry, addWeightEntry } = useApp();
  ...
  const saveWeight = async () => {
    const raw = (weightKgInput || '').trim().replace(',', '.');
    const w = parseFloat(raw);
    if (!Number.isFinite(w) || w <= 0) {
      Alert.alert('Invalid weight', 'Please enter a valid weight in kg (e.g. 72.5).');
      return;
    }
    const ts = initializeWeightTime();
    const result = await addWeightEntry(w, ts);
    if (!result.success) {
      Alert.alert('Save failed', result.error?.message || 'Could not save weight.');
      return;
    }
  ...
  // Handle logging actions from global logging menu
  useEffect(() => {
    const loggingAction = route?.params?.loggingAction;
    if (loggingAction) {
      console.log('📝 Logging action triggered:', loggingAction);
      
      // Use a small delay to ensure screen is ready and modal is closed
      const timeoutId = setTimeout(() => {
        if (loggingAction === 'camera') {
          takePicture();
        } else if (loggingAction === 'gallery') {
          pickImage();
        } else if (loggingAction === 'manual') {
          openManualFoodModal();
        } else if (loggingAction === 'weight') {
          setShowWeightModal(true);
        }
        
        // Clear the flag after action is executed
        navigation.setParams?.({ loggingAction: undefined });
      }, 300);
      
      return () => clearTimeout(timeoutId);
    }
  }, [route?.params?.loggingAction]);

  // Also listen for navigation events to catch param changes when already on screen
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      const loggingAction = route?.params?.loggingAction;
      if (loggingAction) {
        console.log('📝 Logging action triggered on focus:', loggingAction);
    ...
    return unsubscribe;
  }, [navigation, route?.params?.loggingAction]);
  ...
  useEffect(() => {
    if (isAnalyzing) {
      // Rotation animation was removed (we use a static Chick2.png icon now),
      // but older code paths still referenced rotateAnimation in release builds.
      // Keep this as an explicit no-op to avoid crashes.
      const rotateAnimation = { start: () => {}, stop: () => {} } as any;
  ...
  const analyzeAndLogFood = async (imageUri: string) => {
    ...
    try {
      ...
      // Process ingredients directly and show ingredients modal (skip serving-size modal).
      // IMPORTANT: Even if there's only ONE detected item, we still treat it as an ingredient
      // so the ingredients modal is shown immediately after analysis (no "How many servings?" UI).
      {
        console.log(`📋 Processing ${uniqueItems.length} detected item(s) as ingredients...`);

        // Build per-ingredient nutrition (Passio-only).
        // No Edamam fallback: if Passio doesn't provide macros, leave as 0.
        const ingredientsWithNutrition = await Promise.all(
          uniqueItems.map(async (seg: any, index: number) => {
            ...
            // 1) Prefer Passio nutrition (already normalized into FoodAnalysisResult by PassioService)
            const passioN = seg.passioItem;
            const passioHasMacros =
              passioN &&
              typeof passioN === 'object' &&
              (Number(passioN.calories) > 0 ||
                Number(passioN.protein) > 0 ||
                Number(passioN.carbs) > 0 ||
                Number(passioN.fat) > 0);

            if (passioHasMacros) {
              const weightGrams = seg.weightGrams || passioN.servingWeightGrams || 100;
              const nutrition = {
                calories: Math.round(Number(passioN.calories) || 0),
                protein: Math.round((Number(passioN.protein) || 0) * 10) / 10,
                carbs: Math.round((Number(passioN.carbs) || 0) * 10) / 10,
                fat: Math.round((Number(passioN.fat) || 0) * 10) / 10,
                fiber: Math.round((Number(passioN.fiber) || 0) * 10) / 10,
                sugar: Math.round((Number(passioN.sugar) || 0) * 10) / 10,
                weight: weightGrams,
                unit: 'g',
              };

              console.log(`    ✅ Using Passio macros for ${ingName}: ${nutrition.calories} cal`);
            ...
            // Passio did not provide macros for this item
            console.warn(`    ⚠️ No Passio macros for ${ingName}; leaving macros as 0`);
            const weightGrams = seg.weightGrams || passioN?.servingWeightGrams || 100;
            return {
              ...seg,
              nutrition: {
      ...
      if (pendingAnalysis.detectedSegments && pendingAnalysis.detectedSegments.length > 1) {
        ...
        const ingredientsWithNutrition = await Promise.all(
          pendingAnalysis.detectedSegments.map(async (seg: any, index: number) => {
            ...
            // Prefer Passio nutrition (normalized into FoodAnalysisResult by PassioService)
            const passioN = seg.passioItem;
            const passioHasMacros =
              passioN &&
              typeof passioN === 'object' &&
  ...
  const saveHydrationEntry = async () => {
    ...
    // Only allow water to be added through this modal
    await addHydration(volume, 'water');
  ...
  // Helper to get fiber spelling based on user preferences
  const getFiberLabel = () => {
    return state.preferences.units === 'imperial' ? 'Fiber' : 'Fibre';
  };
  ...
  // Helper to format date for modal (e.g., "26th January")
  ...
  const getMealTypeFromTime = (): 'breakfast' | 'lunch' | 'dinner' | 'snack' => {
    ...
    if (hour >= 3 && hour < 9) return 'breakfast';
    if (hour >= 9 && hour < 11) return 'snack';
    if (hour >= 11 && hour < 14) return 'lunch';
    if (hour >= 14 && hour < 16) return 'snack';
    if (hour >= 16 && hour < 22) return 'dinner';
    // 10pm - 3am defaults to dinner
    return 'dinner';
  };
  ...
  const renderMacroProgress = (label: string, current: number, goal: number, unit: string) => {
    ...
    const color = (label.toLowerCase() === 'fiber' || label.toLowerCase() === 'fibre') ? getFiberColor(current) : getProgressColor(percentage, label.toLowerCase() as any);
  ...
  const renderCircularProgressBar = (
    ...
    // Use teal for all progress bars (bowel and symptoms)
    const progressColor = '#0090A3';
    ...
    <SafeAreaView edges={['top', 'left', 'right']} style={styles.container}>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {renderMacroProgress(getFiberLabel(), todaysTotals.fiber, effectiveGoals.fiber, 'g')}
        </View>
        ...
        {/* Weight (stored historically in Supabase; this shows latest for the selected day) */}
        <View style={[styles.card, { padding: 16, marginBottom: 16 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View style={{ flex: 1, paddingRight: 12 }}>
              <Text style={{ fontSize: 16, fontWeight: '700', color: '#2A2A2A' }}>Weight</Text>
              {currentLog?.weight ? (
                <View style={{ marginTop: 4 }}>
                  <Text style={{ color: '#0090A3', fontSize: 16, fontWeight: '600' }}>
                    {currentLog.weight} kg
                  </Text>
                  {(() => {
                    // Find the most recent weight entry BEFORE the selected date
                    const selectedDateObj = new Date(selectedDate);
                    const previousLogs = [...state.dailyLogs]
                      .filter(log => {
                        // Only include logs with weight that are BEFORE the selected date
                        if (log.weight === undefined || log.weight === null) return false;
                    ...
                    if (previousLogs.length > 0) {
                      const previousLog = previousLogs[0]; // Get the most recent entry before selected date
                      const previousWeight = previousLog.weight!;
                      const difference = currentLog.weight - previousWeight;
                      const absDifference = Math.abs(difference);
                      const isLighter = difference < 0;
                      // Parse date string (YYYY-MM-DD) correctly
                      const [year, monthNum, dayNum] = previousLog.date.split('-').map(Number);
                      const day = dayNum;
                      const previousDate = new Date(year, monthNum - 1, dayNum);
                      const month = previousDate.toLocaleDateString('en-US', { month: 'long' });
                      
                      return (
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, flexWrap: 'wrap', marginTop: 4 }}>
                            <MaterialCommunityIcons 
                              name={isLighter ? "arrow-down" : "arrow-up"} 
                              size={16} 
                              color="#666" 
                            />
                            <Text style={{ 
                              ...
                              {absDifference.toFixed(1)} kg
                            </Text>
                            <Text style={{ color: '#666', fontSize: 12 }}>
                              {isLighter ? 'lighter' : 'heavier'} than {day}{getOrdinalSuffix(day)} {month}
                            </Text>
                        </View>
                      );
                    }
            ...
            <TouchableOpacity
              onPress={() => {
                setWeightKgInput(currentLog?.weight ? String(currentLog.weight) : '');
                setShowWeightModal(true);
              }}
              style={{ backgroundColor: '#0090A3', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12 }}
              activeOpacity={0.8}
            >
              <Text style={{ color: '#fff', fontWeight: '700' }}>{currentLog?.weight ? 'Edit' : 'Add'}</Text>
            </TouchableOpacity>
          </View>
        </View>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.manualFoodModal}>
              ...
              <View style={styles.macroRow}>
                ...
                <View style={styles.macroInput}>
                  <Text style={styles.inputLabel}>{getFiberLabel()} (g)</Text>
                  <TextInput
                    style={styles.textInput}
          ...
          <View style={styles.modalOverlay}>
            ...
            >
              <View style={styles.manualFoodModal}>
                ...
                <View style={styles.inputGroup}>
                  <Text style={styles.hydrationInfoText}>
                    If you're logging anything other than plain water, please manually log your drink in your Food Diary!
                  </Text>
                </View>
                ...
                <View style={styles.modalButtons}>
                  ...
                  >
                    <Text style={styles.saveButtonText}>Add Water</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </ScrollView>
          ...
          <View style={styles.modalOverlay}>
            <ScrollView contentContainerStyle={styles.modalScrollContent} keyboardShouldPersistTaps="handled">
              <View style={styles.manualFoodModal}>
                ...
                <Text style={styles.confirmFoodHint}>Enter your weight for {formatDateForModal(selectedDate)}.</Text>
            ...
            <View style={styles.portionModalWrapper}>
              ...
              >
                <View style={styles.portionModalContent}>
                  {/* Ingredients list (shown immediately after analysis) */}
                  {(pendingAnalysis?.ingredients && pendingAnalysis.ingredients.length > 0) ? (
                    <View style={styles.inputGroup}>
          ...
          <View style={styles.loadingContent}>
            ...
            >
              ...
              >
                ...
                >
                  <Image source={ANALYSIS_ICON} style={styles.loadingIconImage} />
                </LinearGradient>
              </Animated.View>
            </Animated.View>
